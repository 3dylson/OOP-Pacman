...
public class Labyrinth {
  ...
  public Labyrinth(Skeleton skel, Level level, int _NumberOfMonsters, int _NumberOfTalented) {
    parent_Level = level;
  ...
  public void AddSurprise(int type) {
    ...
    if (type == 0) {
      Surprise_List[0] = new Bonus(parent_Skeleton);
    }
    if (type == 1) {
      Surprise_List[0] = new Elixir(parent_Skeleton);
    }
    if (type == 2) {
      Surprise_List[0] = new Bomb(parent_Skeleton, false);
    }
    if (type == 4) {
      Surprise_List[0] = new Bomb(parent_Skeleton, true);
    }
  ...
  public void KillDiamond() {
    parent_Skeleton.printp("summon     : Labyrinth:KillDiamond");
    Diamond_List[0].Kill();
    parent_Skeleton.printn("summon end : Labyrinth:KillDiamond");
  }
  ...
  public void CheckCollision() {
    ...
    while ((ExitStatus == false) && (parent_Level.LevelCompleted == false)
        && (parent_Level.parent_GameSet.GameOver == false)) {
      System.out.println("\nPath Simulation.");
      System.out.println("1. Wrong way");
      ...
      case 6: {
        ...
        KillSurprise("Bomb");
        KillMonster();
        parent_Skeleton.printn("it crashes end    : Monster and Bomb(active)");
        break;
      }
